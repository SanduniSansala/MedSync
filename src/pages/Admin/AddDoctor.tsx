...
const AddDoctor: React.FC = () => {
  ...
  const [validationErrors, setValidationErrors] = useState<{ email?: string; contactNumber?: string }>({});
  ...
  const validateFields = () => {
    let errors: { email?: string; contactNumber?: string } = {};
    ...
    if (!phonePattern.test(doctor.contactNumber)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add success handling logic here (e.g., redirect or notification)
    } catch (error) {
      ...
      <div className="min-h-screen bg-gradient-to-r from-blue-100 to-purple-50 py-8">
        <div className="max-w-md mx-auto">
          ...
          <form onSubmit={handleSubmit} className="bg-white/30 backdrop-blur-sm p-8 rounded-xl shadow-2xl border border-blue-100">
            <div className="mb-4">
              <label className="block text-gray-700 text-sm font-medium mb-2">Doctor ID</label>
              <input
                type="text"
                name="doctorID"
            ...
            <div className="mb-4">
              ...
              <select
                name="specialty"
                className="w-full border border-gray-300 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out"
